//! Application service for ChannelMember entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `channel_member_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::domain::entity::ChannelMember;
use crate::infrastructure::persistence::ChannelMemberRepository;
use crate::presentation::dto::{CreateChannelMemberDto, UpdateChannelMemberDto};

/// Application service for ChannelMember entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `channel_member_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ChannelMemberServiceCustom {
///     inner: Arc<ChannelMemberService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ChannelMemberService = GenericCrudService<
    ChannelMember,
    CreateChannelMemberDto,
    UpdateChannelMemberDto,
    ChannelMemberRepository,
>;

// <<< CUSTOM
// END CUSTOM
