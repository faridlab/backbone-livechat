//! Application service for ExpertiseTag entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `expertise_tag_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateExpertiseTagDto, UpdateExpertiseTagDto};
use crate::domain::entity::ExpertiseTag;
use crate::infrastructure::persistence::ExpertiseTagRepository;

/// Application service for ExpertiseTag entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `expertise_tag_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ExpertiseTagServiceCustom {
///     inner: Arc<ExpertiseTagService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ExpertiseTagService = GenericCrudService<
    ExpertiseTag,
    CreateExpertiseTagDto,
    UpdateExpertiseTagDto,
    ExpertiseTagRepository,
>;

// <<< CUSTOM
// END CUSTOM
