//! Application service for ChannelRule entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `channel_rule_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::domain::entity::ChannelRule;
use crate::infrastructure::persistence::ChannelRuleRepository;
use crate::presentation::dto::{CreateChannelRuleDto, UpdateChannelRuleDto};

/// Application service for ChannelRule entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `channel_rule_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ChannelRuleServiceCustom {
///     inner: Arc<ChannelRuleService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ChannelRuleService = GenericCrudService<
    ChannelRule,
    CreateChannelRuleDto,
    UpdateChannelRuleDto,
    ChannelRuleRepository,
>;

// <<< CUSTOM
// END CUSTOM
