//! Domain policy for LivechatAuditLog aggregate
//!
...
/// Domain policy for LivechatAuditLog — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<LivechatAuditLog>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type LivechatAuditLogDomainPolicy = PermitAllPolicy<LivechatAuditLog>;
