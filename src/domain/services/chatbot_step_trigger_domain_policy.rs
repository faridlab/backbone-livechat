//! Domain policy for ChatbotStepTrigger aggregate
//!
...
/// Domain policy for ChatbotStepTrigger — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ChatbotStepTrigger>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ChatbotStepTriggerDomainPolicy = PermitAllPolicy<ChatbotStepTrigger>;
