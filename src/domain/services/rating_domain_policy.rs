//! Domain policy for Rating aggregate
//!
...
/// Domain policy for Rating — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Rating>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type RatingDomainPolicy = PermitAllPolicy<Rating>;
