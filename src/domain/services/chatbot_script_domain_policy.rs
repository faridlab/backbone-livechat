//! Domain policy for ChatbotScript aggregate
//!
...
/// Domain policy for ChatbotScript — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ChatbotScript>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ChatbotScriptDomainPolicy = PermitAllPolicy<ChatbotScript>;
