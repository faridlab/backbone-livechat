...
/// Domain policy for Session — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Session>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type SessionDomainPolicy = PermitAllPolicy<Session>;
