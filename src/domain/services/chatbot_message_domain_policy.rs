//! Domain policy for ChatbotMessage aggregate
//!
...
/// Domain policy for ChatbotMessage — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ChatbotMessage>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ChatbotMessageDomainPolicy = PermitAllPolicy<ChatbotMessage>;
