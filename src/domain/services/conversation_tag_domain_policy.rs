//! Domain policy for ConversationTag aggregate
//!
...
/// Domain policy for ConversationTag — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ConversationTag>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ConversationTagDomainPolicy = PermitAllPolicy<ConversationTag>;
