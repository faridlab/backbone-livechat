-- Channel Seed Data
-- Generated by metaphor-schema
--
-- This file contains initial seed data for the livechat.channels table.
-- Customize this file to add your initial data.

-- INSERT INTO livechat.channels (
--     id,
--     name,
--     website_id,
--     button_text,
--     welcome_message,
--     max_sessions_mode,
--     max_sessions,
--     block_assignment_during_call,
--     review_link,
--     is_active,
--     metadata
-- ) VALUES
-- (
--     -- Add values here
-- );

-- Example: Insert reference data
-- Uncomment and modify the following:

-- INSERT INTO livechat.channels (id, code, name, is_active, created_at, updated_at) VALUES
--     (gen_random_uuid(), 'SAMPLE1', 'Sample 1', true, NOW(), NOW()),
--     (gen_random_uuid(), 'SAMPLE2', 'Sample 2', true, NOW(), NOW());

-- <<< CUSTOM SEED DATA >>>
-- Add your custom seed data below

